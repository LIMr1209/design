...
class TaobaoSpider(SeleniumSpider):
    ...
    def start_requests(self):
        # self.update_cookie()
        # self.stringToDict()
        data = self.test_db.taobao.find({'page': self.page, 'is_suc': 0})
            ...
            if choice == '1':
                try:
                    ...
                    if res.status_code != 200 or json.loads(res.content)['code']:
                        ...
                        self.test_db.taobao.update_one({'link': response.url}, {"$set": {'is_suc': 1}})
                        self.suc_count += 1
            ...
            if choice == '1':
                try:
                    ...
                    if res.status_code != 200 or json.loads(res.content)['code']:
                        ...
                        self.test_db.taobao.update_one({'link': data[0]['link']}, {"$set": {'is_suc': 1}})
                        self.suc_count += 1
